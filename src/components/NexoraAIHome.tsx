...
import { ArrowUp, Bot, Code2, FolderKanban, Image as ImageIcon, Menu, MessageSquare, Plus, Sparkles, Wrench, X, Crown, LayoutGrid, Bug, LogIn, Settings, UserCircle, HelpCircle } from 'lucide-react';
...
function Sidebar({ open, close }: { open: boolean; close: () => void }) {
  ...
  return <>
    ...
    <aside className="fixed inset-y-0 left-0 z-[70] flex w-[min(340px,88vw)] flex-col border-r border-white/[0.07] bg-[#070707]/96 shadow-2xl shadow-black/70 backdrop-blur-2xl">
      <div className="flex h-16 shrink-0 items-center justify-between border-b border-white/[0.06] px-4"><div className="flex items-center gap-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#c65a16] text-black shadow-lg shadow-orange-950/30"><Sparkles className="h-4 w-4" /></div><span className="text-sm font-black tracking-tight text-zinc-100">NexoraTools</span></div><button type="button" onClick={close} className="rounded-xl p-2 text-zinc-500 transition hover:bg-white/[0.04] hover:text-zinc-100" aria-label="Tutup menu"><X className="h-4 w-4" /></button></div>
      <nav className="flex-1 overflow-y-auto p-3"><div className="mb-2 px-2 text-[10px] font-bold uppercase tracking-[0.16em] text-zinc-600">Workspace</div>
        ...
        <Link href="/nexora-ai" onClick={close} className="mb-1 flex items-center gap-3 rounded-xl border border-[#c65a16]/20 bg-[#c65a16]/[0.07] px-3 py-2.5 text-xs font-bold text-[#d66a25]"><Sparkles className="h-4 w-4" /> NexoraAI <span className="ml-auto rounded-md bg-[#c65a16]/10 px-1.5 py-0.5 text-[8px] uppercase tracking-wider text-[#d66a25]">AI</span></Link>
        ...
        <Link href="/help" onClick={close} className="mb-1 flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-semibold text-zinc-400 transition hover:bg-white/[0.05] hover:text-zinc-100"><HelpCircle className="h-4 w-4" /> Bantuan</Link>
        <Link href="/feedback" onClick={close} className="mb-1 flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-semibold text-zinc-400 transition hover:bg-white/[0.05] hover:text-zinc-100"><Bug className="h-4 w-4" /> Lapor Bug / Saran</Link>
      </nav>
      <div className="border-t border-white/[0.06] p-3"><div className="grid grid-cols-3 gap-1"><Link href="/profile" onClick={close} className="flex flex-col items-center gap-1 rounded-xl px-2 py-2.5 text-[10px] font-semibold text-zinc-500 transition hover:bg-white/[0.05] hover:text-zinc-100"><UserCircle className="h-4 w-4" />Profil</Link><Link href="/settings" onClick={close} className="flex flex-col items-center gap-1 rounded-xl px-2 py-2.5 text-[10px] font-semibold text-zinc-500 transition hover:bg-white/[0.05] hover:text-zinc-100"><Settings className="h-4 w-4" />Pengaturan</Link><Link href="/login" onClick={close} className="flex flex-col items-center gap-1 rounded-xl px-2 py-2.5 text-[10px] font-semibold text-zinc-500 transition hover:bg-white/[0.05] hover:text-zinc-100"><LogIn className="h-4 w-4" />Masuk</Link></div></div>
    </aside>
  </>;
}
