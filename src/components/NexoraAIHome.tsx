  ...
  return <>
    ...
    <aside className="fixed inset-y-0 left-0 z-[70] flex w-[min(340px,88vw)] flex-col border-r border-white/[0.07] bg-[#070707]/96 shadow-2xl shadow-black/70 backdrop-blur-2xl">
      ...
      <nav className="flex-1 overflow-y-auto p-3">
        ...
        <button type="button" onClick={() => { close(); window.dispatchEvent(new CustomEvent('nexora-feedback')); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-semibold text-zinc-400 transition hover:bg-white/[0.05] hover:text-zinc-100"><Bug className="h-4 w-4 text-[#c65a16]" /> Lapor Bug / Saran</button>
      </nav>
...
export default function NexoraAIHome() {
  ...
  return <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden bg-[#050505] text-[#f4f4f5] selection:bg-[#c65a16]/20">
    ...
    <button type="button" onClick={() => setMenuOpen(true)} className="press-glow fixed right-4 top-4 z-50 flex h-10 w-10 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] p-2 text-zinc-400 shadow-xl shadow-black/20 backdrop-blur-xl hover:border-[#c65a16]/25 hover:text-zinc-100 md:hidden" aria-label="Buka menu NexoraTools"><Menu className="h-5 w-5" /></button>
