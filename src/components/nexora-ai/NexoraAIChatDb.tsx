...
export default function NexoraAIChatDb() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-[#c65a16]/30">
      <header className="sticky top-0 z-30 bg-[#050505]/75 p-4 backdrop-blur-xl"><button type="button" onClick={() => setMenuOpen(true)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.025] text-zinc-500 shadow-sm transition hover:border-white/15 hover:bg-white/[0.05] hover:text-zinc-100" aria-label="Buka menu chat"><Menu className="h-5 w-5" /></button></header>
      <ChatSidebar open={menuOpen} conversations={conversations} activeId={activeId} loadingHistory={loadingHistory} onClose={() => setMenuOpen(false)} onNewChat={() => void newChat()} onSelect={(id) => { setActiveId(id); setMenuOpen(false); }} onDelete={(id) => void deleteChat(id)} />
      <main className="mx-auto flex min-h-[calc(100vh-4.5rem)] max-w-3xl flex-col px-4 sm:px-6">
